          ...
          <section className="min-w-0 rounded-lg border bg-card p-4 shadow-sm sm:p-5">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
              ...
              <label className="min-w-0 space-y-1">
                ...
                <input type="time" value={startTime} onChange={(event) => setStartTime(event.target.value)} className="nsh-mobile-time h-14 w-full rounded-lg border-2 border-border bg-background font-semibold outline-none focus:border-primary" />
              </label>
              <label className="min-w-0 space-y-1">
                <span className="text-xs font-bold uppercase text-muted-foreground">Bis</span>
                <input type="time" value={endTime} onChange={(event) => setEndTime(event.target.value)} className="nsh-mobile-time h-14 w-full rounded-lg border-2 border-border bg-background font-semibold outline-none focus:border-primary" />
              </label>
              <label className="min-w-0 space-y-1 sm:col-span-2">
              ...
              <div className="space-y-1 sm:col-span-2">
                ...
                <div className="grid min-w-0 grid-cols-3 gap-2">
                  {[0, 1, 2].map((count) => (
                    <button key={count} type="button" onClick={() => setHelpersCount(count)} className={cn("h-14 min-w-0 rounded-lg border-2 text-base font-black", helpersCount === count ? "border-primary bg-primary/10 text-primary" : "border-border bg-background")}>{count}</button>
                  ))}
                </div>
              </div>
            </div>
